...
- `reviewer-typescript` (if `.ts`/`.tsx` in source directories changed)
- `reviewer-eslint` (if `.ts`/`.tsx` in source directories changed)
- `reviewer-tests` (if `.ts`/`.tsx` in source directories changed)
...
**FuryStack Application-Specific:**
