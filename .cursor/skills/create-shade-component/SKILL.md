...
Generate a Shade component that follows `.cursor/rules/CODE_STYLE.mdc`, `.cursor/rules/TYPESCRIPT_GUIDELINES.mdc`, `.cursor/rules/COMPLEXITY.mdc`, `.cursor/rules/FRONTEND_PATTERNS.mdc`, and `.cursor/rules/OBSERVABLE_STATE.mdc`. Read those rules first; this skill owns the workflow.
